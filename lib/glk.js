...
var _glkConstants = require("./glkConstants");
...
function filePath(filename) {
  var vorpleFiles = [_file.HANDSHAKE_FILENAME, _file.JS_EVAL_FILENAME, _file.JS_RETURN_VALUE_FILENAME, _file.JS_RETURN_VALUE_TYPE_FILENAME];

  if (vorpleFiles.indexOf(filename) > -1) {
    return _file.VORPLE_PATH;
  }

  return _file.INFORM_PATH;
}
/**
...
function handle_char_input(
/* disprock, */
input) {
  // modified
  var charval;
  if (!gli_selectref) return;
  var win = gli_rootwin; // we have only one window

  if (input.length == 1) {
    charval = input.charCodeAt(0);
    if (!win.char_request_uni) charval = charval & 0xFF;
  } else {
    charval = _glkConstants.KeystrokeNameMap[input];
    if (!charval) charval = _glkConstants.Const.keycode_Unknown;
  }

  gli_selectref.set_field(0, _glkConstants.Const.evtype_CharInput);
  ...
  if (window.GiDispa) GiDispa.prepare_resume(gli_selectref);
  gli_selectref = null;
  VM.resume();
}

function handle_line_input(
/* disprock, */
input, _termkey) {
  // modified
  var ix;
  ...
  var win = gli_rootwin;
  if (input.length > win.linebuf.length) input = input.slice(0, win.linebuf.length);

  if (win.request_echo_line_input) {
    ix = win.style;
    gli_set_style(win.str, _glkConstants.Const.style_Input);
    gli_window_put_string(win, input);
    if (win.echostr) glk_put_jstring_stream(win.echostr, input);
  ...
  var termcode = _glkConstants.Const.keycode_Return;
  /*
  if (termkey && KeystrokeNameMap[termkey]) 
      termcode = KeystrokeNameMap[termkey];
  */

  gli_selectref.set_field(0, _glkConstants.Const.evtype_LineInput);
  gli_selectref.set_field(1, win);
  gli_selectref.set_field(2, input.length);
  gli_selectref.set_field(3, termcode);
  if (window.GiDispa) GiDispa.unretain_array(win.linebuf);
...
function update() {
  ...
  if (useobj) contentarray.push(obj);
  inputarray = [];
  obj = null;
    ...
    switch (ui_specialinput.filetype) {
      case 'save':
        if (ui_specialinput.filemode === "write") {
          (0, _file.saveFilePrompt)(ui_specialinput.gameid, callback);
        } else {
          (0, _file.restoreFilePrompt)(ui_specialinput.gameid, callback);
        }

        break;

      case 'transcript':
        (0, _file.transcriptFilePrompt)(callback);
        break;

      default:
        (0, _file.filePrompt)(callback);
        break;
    }
  }
  ...
  if (gli_autorestore_glkstate) dataobj.autorestore = gli_autorestore_glkstate;
  gli_autorestore_glkstate = null; // GlkOte.update(dataobj, gli_autorestore_glkstate);
  ...
  if (option_do_vm_autosave) {
    if (has_exited) {
      ...
      if (eventarg) VM.do_autosave(eventarg);
    }
  }
}
...
function TrimArrayToBytes(arr) {
  ...
  for (ix = 0; ix < len; ix++) {
    if (arr[ix] < 0 || arr[ix] >= 0x100) break;
  }
  ...
  for (ix = 0; ix < len; ix++) {
    if (arr[ix] < 0 || arr[ix] >= 0x100) newarr[ix] = 63; // '?'
    else newarr[ix] = arr[ix];
  }
...
function call_may_not_return(id) {
  if (id == 0x001 || id == 0x0C0 || id == 0x062) return true;else return false;
}
...
function gli_new_window(type, rock) {
  ...
  win.echostr = null;
  win.style = _glkConstants.Const.style_Normal;
  win.hyperlink = 0;
  win.input_generation = null;
  win.linebuf = null;
  win.char_request = false;
  ...
  gli_windowlist = win;
  if (win.next) win.next.prev = win;
  if (window.GiDispa) GiDispa.class_register('window', win);else win.disprock = gli_api_display_rocks++;
...
function gli_delete_window(win) {
  var prev, next;
  if (window.GiDispa) GiDispa.class_unregister('window', win);
  geometry_changed = true;
  win.echostr = null;
  ...
  win.prev = null;
  win.next = null;
  if (prev) prev.next = next;else gli_windowlist = next;
...
function gli_windows_unechostream(str) {
  ...
  for (win = gli_windowlist; win; win = win.next) {
    if (win.echostr === str) win.echostr = null;
  }
}

function gli_window_put_string(_win, val) {
  // modified
  (0, _buffer.append)(val);
}

function gli_window_buffer_deaccumulate(win) {
  var conta = win.content;
  var stylename = _glkConstants.StyleNameMap[win.accumstyle];
...
function gli_window_buffer_put_special(win, special, flowbreak) {
  ...
  if (conta.length == 0) {
    ...
    if (flowbreak) obj.flowbreak = true;
    conta.push(obj);
  } else {
    obj = conta[conta.length - 1];
    if (flowbreak) obj.flowbreak = true;
...
function gli_new_stream(type, readable, writable, rock) {
  ...
  if (window.GiDispa) GiDispa.class_register('stream', str);
  return str;
}
...
function gli_delete_stream(str) {
  ...
  if (str.type == strtype_Memory) {
    if (window.GiDispa) GiDispa.unretain_array(str.buf);
  ...
  if (window.GiDispa) GiDispa.class_unregister('stream', str);
  prev = str.prev;
  next = str.next;
  str.prev = null;
  str.next = null;
  if (prev) prev.next = next;else gli_streamlist = next;
  if (next) next.prev = prev;
  str.fstream = null;
  str.buf = null;
  str.readable = false;
...
function gli_stream_dirty_file(str) {
  if (str.streaming) GlkOte.log('### gli_stream_dirty_file called for streaming file!');

  if (str.timer_id === null) {
    if (str.flush_func === null) {
      /* Bodge together a closure to act as a stream method. */
      str.flush_func = function () {
...
function gli_stream_flush_file(str) {
  if (str.streaming) GlkOte.log('### gli_stream_flush_file called for streaming file!');
  ...
  str.timer_id = null;
  (0, _file.write)(str.ref.filename, str.buf, {
    append: true
  });
}
...
function gli_new_fileref(filename, usage, rock, ref) {
  ...
  fref.textmode = (usage & _glkConstants.Const.fileusage_TextMode) != 0;
  fref.filetype = usage & _glkConstants.Const.fileusage_TypeMask;
  fref.filetypename = _glkConstants.FileTypeMap[fref.filetype];
  ...
  if (!ref) {
    var gameid = '';
    if (fref.filetype == _glkConstants.Const.fileusage_SavedGame) gameid = VM.get_signature(); // ref = Dialog.file_construct_ref(fref.filename, fref.filetypename, gameid);
  }
  ...
  if (fref.next) fref.next.prev = fref;
  if (window.GiDispa) GiDispa.class_register('fileref', fref);
  return fref;
}

function gli_delete_fileref(fref) {
  var prev, next;
  if (window.GiDispa) GiDispa.class_unregister('fileref', fref);
  prev = fref.prev;
  next = fref.next;
  fref.prev = null;
  fref.next = null;
  if (prev) prev.next = next;else gli_filereflist = next;
  if (next) next.prev = prev;
  fref.filename = null;
  fref.ref = null;
  fref.rock = null;
...
function gli_put_char(str, ch) {
  if (!str || !str.writable) throw 'gli_put_char: invalid stream';

  if (!str.unicode) {
    if (ch < 0 || ch >= 0x100) ch = 63; // '?'
  }
  ...
  switch (str.type) {
    case strtype_File:
      ...
      if (!str.unicode || ch < 0x80 && !str.isbinary) {
        if (str.bufpos < str.buflen) {
          ...
          str.bufpos += 1;
          if (str.bufpos > str.bufeof) str.bufeof = str.bufpos;
        }
      } else {
        var arr;
        if (!str.isbinary) arr = UniArrayToUTF8([ch]);else arr = UniArrayToBE32([ch]);
        var len = arr.length;
        if (len > str.buflen - str.bufpos) len = str.buflen - str.bufpos;
    ...
    case strtype_Window:
      if (str.win.line_request) throw 'gli_put_char: window has pending line request';
      gli_window_put_string(str.win, CharToString(ch));
      if (str.win.echostr) gli_put_char(str.win.echostr, ch);
      break;
  }
}
...
function gli_put_array(str, arr, allbytes) {
  var ix, len, val;
  if (!str || !str.writable) throw 'gli_put_array: invalid stream';
  ...
  switch (str.type) {
    case strtype_File:
      if (str.streaming) {
        ...
        if (!str.unicode) {
          ...
          if (!str.isbinary) arr8 = UniArrayToUTF8(arr);else arr8 = UniArrayToBE32(arr);
        }
    ...
    case strtype_Window:
      if (str.win.line_request) throw 'gli_put_array: window has pending line request';
      if (allbytes) val = String.fromCharCode.apply(this, arr);else val = UniArrayToString(arr);
      gli_window_put_string(str.win, val);
      if (str.win.echostr) gli_put_array(str.win.echostr, arr, allbytes);
      break;
  }
}
  ...
  switch (str.type) {
    case strtype_File:
      if (str.streaming) {
        if (!str.unicode) {
          ...
          str.readcount++;
          ch >>>= 0;
          if (!want_unicode && ch >= 0x100) return 63; // return '?'
    ...
    case strtype_Resource:
      if (str.unicode) {
        if (str.isbinary) {
          /* cheap big-endian stream */
          if (str.bufpos >= str.bufeof) return -1;
  ...
  switch (str.type) {
    ...
    case strtype_Memory:
      ...
      if (!want_unicode) {
        for (lx = 0; lx < len && !gotnewline; lx++) {
          ch = str.buf[str.bufpos++];
          if (!want_unicode && ch >= 0x100) ch = 63; // ch = '?'
...
function glk_put_jstring_stream(str, val, allbytes) {
  var ix, len;
  if (!str || !str.writable) throw 'glk_put_jstring: invalid stream';
  ...
  switch (str.type) {
    case strtype_File:
      if (str.streaming) {
        if (!str.unicode) {
          ...
          if (!str.isbinary) {
            /* cheap UTF-8 stream */
            var _buf = new str.fstream.BufferClass(val); // utf8


            str.fstream.fwrite(_buf);
          } else {
            /* cheap big-endian stream */
            var _buf2 = new str.fstream.BufferClass(4 * val.length);

            for (ix = 0; ix < val.length; ix++) {
              _buf2.writeUInt32BE(val.charCodeAt(ix), 4 * ix, true);
            }

            str.fstream.fwrite(_buf2);
          }
        }
      } else {
        /* non-streaming... */
        ...
        var _len = arr8.length;
        if (_len > str.buflen - str.bufpos) _len = str.buflen - str.bufpos;
    ...
    case strtype_Window:
      if (str.win.line_request) throw 'glk_put_jstring: window has pending line request';
      gli_window_put_string(str.win, val);
      if (str.win.echostr) glk_put_jstring_stream(str.win.echostr, val, allbytes);
      break;
  }
}

function gli_set_style(str, val) {
  if (!str || !str.writable) throw 'gli_set_style: invalid stream';
  if (val >= _glkConstants.Const.style_NUMSTYLES) val = 0;

  if (str.type == strtype_Window) {
    (0, _haven2.setStyle)(val, 0);
    if (str.win.echostr) gli_set_style(str.win.echostr, val);
  }
}
...
function glk_gestalt_ext(sel, val, arr) {
  ...
  switch (sel) {
    ...
    case 1:
      ...
      if (val <= _glkConstants.Const.keycode_Left && val >= _glkConstants.Const.keycode_End) return 1;
      if (val >= 0x100000000 - _glkConstants.Const.keycode_MAXVAL) return 0;
    ...
    case 19:
      ...
      if (val == _glkConstants.Const.keycode_Escape) return 1;
      if (val >= _glkConstants.Const.keycode_Func12 && val <= _glkConstants.Const.keycode_Func1) return 1;
      return 0;
...
function glk_window_iterate(win, rockref) {
  if (!win) win = gli_windowlist;else win = win.next;

  if (win) {
    if (rockref) rockref.set_value(win.rock);
    return win;
  }

  if (rockref) rockref.set_value(0);
  return null;
}

function glk_window_get_rock(win) {
  if (!win) throw 'glk_window_get_rock: invalid window';
  return win.rock;
}
...
function glk_window_open(splitwin, _method, _size, wintype, rock) {
  // modified
  var newwin;
  ...
  if (splitwin) throw 'glk_window_open: splitwin must be null for first window';
  newwin = gli_new_window(wintype, rock);

  switch (newwin.type) {
    case _glkConstants.Const.wintype_TextBuffer:
...
function glk_window_get_type(win) {
  if (!win) throw 'glk_window_get_type: invalid window';
  return win.type;
}

function glk_window_get_parent(win) {
  if (!win) throw 'glk_window_get_parent: invalid window';
  return win.parent;
}

function glk_window_get_stream(win) {
  if (!win) throw 'glk_window_get_stream: invalid window';
  return win.str;
}

function glk_window_set_echo_stream(win, str) {
  if (!win) throw 'glk_window_set_echo_stream: invalid window';
  win.echostr = str;
}

function glk_window_get_echo_stream(win) {
  if (!win) throw 'glk_window_get_echo_stream: invalid window';
  return win.echostr;
}

function glk_set_window(win) {
  if (!win) gli_currentstr = null;else gli_currentstr = win.str;
}

function glk_window_get_sibling(win) {
  var parent, sib;
  if (!win) throw 'glk_window_get_sibling: invalid window';
  parent = win.parent;
  if (!parent) return null;
  if (win === parent.child1) return parent.child2;else if (win === parent.child2) return parent.child1;else throw 'glk_window_get_sibling: window tree is corrupted';
}

function glk_stream_iterate(str, rockref) {
  if (!str) str = gli_streamlist;else str = str.next;

  if (str) {
    if (rockref) rockref.set_value(str.rock);
    return str;
  }
...
function glk_stream_get_rock(str) {
  if (!str) throw 'glk_stream_get_rock: invalid stream';
  return str.rock;
}
...
function glk_stream_open_file(fref, fmode, rock) {
  ...
  var filename = fref.filename;
  var content = null;

  if (fmode !== _glkConstants.Const.filemode_Write) {
    content = (0, _file.read)(filename, {
  ...
  if (content === null) {
    ...
    if (fmode !== _glkConstants.Const.filemode_Read) {
  ...
  var str = gli_new_stream(strtype_File, fmode != _glkConstants.Const.filemode_Write, fmode != _glkConstants.Const.filemode_Read, rock);
  str.unicode = false;
  ...
  if (typeof content === 'string') {
    str.buf = content.split('').map(function (_char) {
  ...
  str.buflen = 0xFFFFFFFF;
  /* enormous */

  if (fmode == _glkConstants.Const.filemode_Write) str.bufeof = 0;else str.bufeof = content.length;
  if (fmode == _glkConstants.Const.filemode_WriteAppend) str.bufpos = str.bufeof;else str.bufpos = 0;
  return str;
}

function glk_stream_open_memory(buf, fmode, rock) {
  var str;
  if (fmode != _glkConstants.Const.filemode_Read && fmode != _glkConstants.Const.filemode_Write && fmode != _glkConstants.Const.filemode_ReadWrite) throw 'glk_stream_open_memory: illegal filemode';
  str = gli_new_stream(strtype_Memory, fmode != _glkConstants.Const.filemode_Write, fmode != _glkConstants.Const.filemode_Read, rock);
  str.unicode = false;
  ...
  if (buf) {
    ...
    str.bufpos = 0;
    if (fmode == _glkConstants.Const.filemode_Write) str.bufeof = 0;else str.bufeof = str.buflen;
    if (window.GiDispa) GiDispa.retain_array(buf);
  }
...
function glk_stream_close(str, result) {
  if (!str) throw 'glk_stream_close: invalid stream';
  if (str.type == strtype_Window) throw 'glk_stream_close: cannot close window stream';
  ...
  if (str.type == strtype_File && str.writable) {
    if (!str.streaming) {
      ...
      var contents = str.buf.map(function (code) {
        return String.fromCharCode(code);
      }).join('');
      var code = contents.substring(contents.indexOf('\n') + 1);

      if (str.filename === 'VpJSEval' && code.length > 1) {
        (0, _vorple.evaluate)(code);
      } else {
        (0, _file.write)(str.filename, str.buf, {
          binary: str.isbinary,
...
function glk_stream_set_position(str, pos, seekmode) {
  if (!str) throw 'glk_stream_set_position: invalid stream';
  ...
  switch (str.type) {
    ...
    case strtype_Memory:
      if (seekmode == _glkConstants.Const.seekmode_Current) {
        pos = str.bufpos + pos;
      } else if (seekmode == _glkConstants.Const.seekmode_End) {
        pos = str.bufeof + pos;
      } else {
        /* pos = pos */
      }
      ...
      if (pos < 0) pos = 0;
      if (pos > str.bufeof) pos = str.bufeof;
      str.bufpos = pos;
  }
}

function glk_stream_get_position(str) {
  if (!str) throw 'glk_stream_get_position: invalid stream';
...
function glk_fileref_create_by_prompt(usage, fmode, rock) {
  var modename;
  var filetype = usage & _glkConstants.Const.fileusage_TypeMask;
  var filetypename = _glkConstants.FileTypeMap[filetype];
  ...
  switch (fmode) {
    case _glkConstants.Const.filemode_Write:
      modename = 'write';
      break;

    case _glkConstants.Const.filemode_ReadWrite:
      modename = 'readwrite';
      break;

    case _glkConstants.Const.filemode_WriteAppend:
      modename = 'writeappend';
      break;

    case _glkConstants.Const.filemode_Read:
    default:
      modename = 'read';
      break;
  }
  ...
  if (filetype == _glkConstants.Const.fileusage_SavedGame) special.gameid = VM.get_signature();
...
function gli_fileref_create_by_prompt_callback(obj) {
  ...
  if (window.GiDispa) GiDispa.prepare_resume(fref);
  VM.resume(fref);
}

function glk_fileref_destroy(fref) {
  if (!fref) throw 'glk_fileref_destroy: invalid fileref';
  gli_delete_fileref(fref);
}

function glk_fileref_iterate(fref, rockref) {
  if (!fref) fref = gli_filereflist;else fref = fref.next;

  if (fref) {
    if (rockref) rockref.set_value(fref.rock);
    return fref;
  }
...
function glk_fileref_get_rock(fref) {
  if (!fref) throw 'glk_fileref_get_rock: invalid fileref';
  return fref.rock;
}
...
function glk_fileref_does_file_exist(fref) {
  ...
  return (0, _file.exists)(fref.filename, {
    cwd: filePath(fref.filename)
  }) ? 1 : 0;
}

function glk_fileref_create_from_fileref(usage, oldfref, rock) {
  if (!oldfref) throw 'glk_fileref_create_from_fileref: invalid fileref';
...
function glk_get_char_stream(str) {
  if (!str) throw 'glk_get_char_stream: invalid stream';
  return gli_get_char(str, false);
}

function glk_get_line_stream(str, buf) {
  if (!str) throw 'glk_get_line_stream: invalid stream';
  return gli_get_line(str, buf, false);
}

function glk_get_buffer_stream(str, buf) {
  if (!str) throw 'glk_get_buffer_stream: invalid stream';
  return gli_get_buffer(str, buf, false);
}

function glk_char_to_lower(val) {
  if (val >= 0x41 && val <= 0x5A) return val + 0x20;
  if (val >= 0xC0 && val <= 0xDE && val != 0xD7) return val + 0x20;
  return val;
}

function glk_char_to_upper(val) {
  if (val >= 0x61 && val <= 0x7A) return val - 0x20;
  if (val >= 0xE0 && val <= 0xFE && val != 0xF7) return val - 0x20;
  return val;
}
...
function glk_request_line_event(_win, buf, initlen) {
  // modified
  ...
  if (initlen) {
    ...
    if (!current_partial_outputs) current_partial_outputs = {};
    current_partial_outputs[win.disprock] = ByteArrayToString(ls);
  }

  win.line_request = true;
  win.line_request_uni = false;
  if (win.type == _glkConstants.Const.wintype_TextBuffer) win.request_echo_line_input = win.echo_line_input;else win.request_echo_line_input = true;
  win.input_generation = event_generation;
  win.linebuf = buf;
  if (window.GiDispa) GiDispa.retain_array(buf);
  (0, _buffer.flush)();
  (0, _prompt.expectInput)();
}

function glk_request_char_event(_win) {
  // modified
  var win = gli_rootwin;
  if (win.char_request || win.line_request) throw 'glk_request_char_event: window already has keyboard request';
  win.char_request = true;
  win.char_request_uni = false;
  win.input_generation = event_generation;
  (0, _buffer.flush)();

  _input.keypress.wait();
}

function glk_cancel_char_event(win) {
  if (!win) throw 'glk_cancel_char_event: invalid window';
  win.char_request = false;
  win.char_request_uni = false;
}

function glk_set_echo_line_event(win, val) {
  if (!win) throw 'glk_set_echo_line_event: invalid window';
...
function glk_set_terminators_line_event(win, arr) {
  if (!win) throw 'glk_set_terminators_line_event: invalid window';
  ...
  if (KeystrokeValueMap === null) {
    ...
    for (var val in _glkConstants.KeystrokeNameMap) {
      KeystrokeValueMap[_glkConstants.KeystrokeNameMap[val]] = val;
    }
  }
...
function glk_buffer_to_lower_case_uni(arr, numchars) {
  ...
  if (arrlen < numchars) throw 'buffer_to_lower_case_uni: numchars exceeds array length';
  pos = 0;

  for (ix = 0; ix < numchars; ix++) {
    origval = src[ix];
    val = _glkConstants.unicode_lower_table[origval];
...
function glk_buffer_to_upper_case_uni(arr, numchars) {
  ...
  var arrlen = arr.length;
  var src = arr.slice(0, numchars);
  if (arrlen < numchars) throw 'buffer_to_upper_case_uni: numchars exceeds array length';
  pos = 0;

  for (ix = 0; ix < numchars; ix++) {
    origval = src[ix];
    val = _glkConstants.unicode_upper_table[origval];
...
function glk_buffer_to_title_case_uni(arr, numchars, lowerrest) {
  ...
  var arrlen = arr.length;
  var src = arr.slice(0, numchars);
  if (arrlen < numchars) throw 'buffer_to_title_case_uni: numchars exceeds array length';
  pos = 0;
  if (numchars == 0) return 0;
  ix = 0;
  {
    origval = src[ix];
    val = _glkConstants.unicode_title_table[origval];
...
function gli_buffer_canon_decompose_uni(arr, numchars) {
  ...
  for (ix = 0; ix < numchars; ix++) {
    origval = src[ix];
    val = _glkConstants.unicode_decomp_table[origval];
  ...
  while (ix < pos) {
    if (!_glkConstants.unicode_combin_table[arr[ix]]) {
      ix++;
      continue;
    }
    ...
    while (ix < pos && _glkConstants.unicode_combin_table[arr[ix]]) {
      ix++;
    }
    ...
    if (grpend - grpstart >= 2) {
      ...
      for (jx = grpend - 1; jx > grpstart; jx--) {
        for (kx = grpstart; kx < jx; kx++) {
          if (_glkConstants.unicode_combin_table[arr[kx]] > _glkConstants.unicode_combin_table[arr[kx + 1]]) {
            tmp = arr[kx];
            arr[kx] = arr[kx + 1];
            arr[kx + 1] = tmp;
          }
...
function gli_buffer_canon_compose_uni(arr, numchars) {
  ...
  pos = 0;
  curch = arr[0];
  curclass = _glkConstants.unicode_combin_table[curch];
  if (curclass) curclass = 999; // just in case the first character is a combiner
  ...
  while (true) {
    ...
    newch = arr[jx];
    newclass = _glkConstants.unicode_combin_table[newch];
    map = _glkConstants.unicode_compo_table[curch];

    if (map !== undefined && map[newch] !== undefined && (!curclass || newclass && curclass < newclass)) {
      curch = map[newch];
      arr[pos] = curch;
    } else {
      if (!newclass) {
...
function glk_get_char_stream_uni(str) {
  if (!str) throw 'glk_get_char_stream_uni: invalid stream';
  return gli_get_char(str, true);
}

function glk_get_buffer_stream_uni(str, buf) {
  if (!str) throw 'glk_get_buffer_stream_uni: invalid stream';
  return gli_get_buffer(str, buf, true);
}

function glk_get_line_stream_uni(str, buf) {
  if (!str) throw 'glk_get_line_stream_uni: invalid stream';
...
function glk_request_char_event_uni(win) {
  if (!win) throw 'glk_request_char_event: invalid window';
  if (win.char_request || win.line_request) throw 'glk_request_char_event: window already has keyboard request';
  win.char_request = true;
...
function glk_request_line_event_uni(win, buf, initlen) {
  ...
  if (!win) throw 'glk_request_line_event: invalid window';
  if (win.char_request || win.line_request) throw 'glk_request_line_event: window already has keyboard request';
...
function glk_date_to_time_local(dateref, timevalref) {
  var obj = new Date(dateref.get_field(0), dateref.get_field(1) - 1, dateref.get_field(2), dateref.get_field(4), dateref.get_field(5), dateref.get_field(6), dateref.get_field(7) / 1000);
  var now = obj.getTime();
  var usec;
  timevalref.set_field(0, Math.floor(now / 4294967296000));
  timevalref.set_field(1, Math.floor(now / 1000) >>> 0);
...
var GLK = (_GLK = {
  ...
  /* GlkOte/GlkApi version */
  init: init,
  update: update,
  // save_allstate: save_allstate,
  // restore_allstate: restore_allstate,
  fatal_error: fatal_error
}, (0, _defineProperty2["default"])(_GLK, "fatal_error", fatal_error), (0, _defineProperty2["default"])(_GLK, "byte_array_to_string", ByteArrayToString), (0, _defineProperty2["default"])(_GLK, "uni_array_to_string", UniArrayToString), (0, _defineProperty2["default"])(_GLK, "Const", _glkConstants.Const), (0, _defineProperty2["default"])(_GLK, "RefBox", RefBox), (0, _defineProperty2["default"])(_GLK, "RefStruct", RefStruct), (0, _defineProperty2["default"])(_GLK, "DidNotReturn", DidNotReturn), (0, _defineProperty2["default"])(_GLK, "call_may_not_return", call_may_not_return), (0, _defineProperty2["default"])(_GLK, "glk_put_jstring", glk_put_jstring), (0, _defineProperty2["default"])(_GLK, "glk_put_jstring_stream", glk_put_jstring_stream), (0, _defineProperty2["default"])(_GLK, "glk_exit", glk_exit), (0, _defineProperty2["default"])(_GLK, "glk_tick", glk_tick), (0, _defineProperty2["default"])(_GLK, "glk_gestalt", glk_gestalt), (0, _defineProperty2["default"])(_GLK, "glk_gestalt_ext", glk_gestalt_ext), (0, _defineProperty2["default"])(_GLK, "glk_window_iterate", glk_window_iterate), (0, _defineProperty2["default"])(_GLK, "glk_window_get_rock", glk_window_get_rock), (0, _defineProperty2["default"])(_GLK, "glk_window_get_root", glk_window_get_root), (0, _defineProperty2["default"])(_GLK, "glk_window_open", glk_window_open), (0, _defineProperty2["default"])(_GLK, "glk_window_get_type", glk_window_get_type), (0, _defineProperty2["default"])(_GLK, "glk_window_get_parent", glk_window_get_parent), (0, _defineProperty2["default"])(_GLK, "glk_window_get_stream", glk_window_get_stream), (0, _defineProperty2["default"])(_GLK, "glk_window_set_echo_stream", glk_window_set_echo_stream), (0, _defineProperty2["default"])(_GLK, "glk_window_get_echo_stream", glk_window_get_echo_stream), (0, _defineProperty2["default"])(_GLK, "glk_set_window", glk_set_window), (0, _defineProperty2["default"])(_GLK, "glk_window_get_sibling", glk_window_get_sibling), (0, _defineProperty2["default"])(_GLK, "glk_stream_iterate", glk_stream_iterate), (0, _defineProperty2["default"])(_GLK, "glk_stream_get_rock", glk_stream_get_rock), (0, _defineProperty2["default"])(_GLK, "glk_stream_open_file", glk_stream_open_file), (0, _defineProperty2["default"])(_GLK, "glk_stream_open_memory", glk_stream_open_memory), (0, _defineProperty2["default"])(_GLK, "glk_stream_close", glk_stream_close), (0, _defineProperty2["default"])(_GLK, "glk_stream_set_position", glk_stream_set_position), (0, _defineProperty2["default"])(_GLK, "glk_stream_get_position", glk_stream_get_position), (0, _defineProperty2["default"])(_GLK, "glk_stream_set_current", glk_stream_set_current), (0, _defineProperty2["default"])(_GLK, "glk_stream_get_current", glk_stream_get_current), (0, _defineProperty2["default"])(_GLK, "glk_fileref_create_by_name", glk_fileref_create_by_name), (0, _defineProperty2["default"])(_GLK, "glk_fileref_create_by_prompt", glk_fileref_create_by_prompt), (0, _defineProperty2["default"])(_GLK, "glk_fileref_destroy", glk_fileref_destroy), (0, _defineProperty2["default"])(_GLK, "glk_fileref_iterate", glk_fileref_iterate), (0, _defineProperty2["default"])(_GLK, "glk_fileref_get_rock", glk_fileref_get_rock), (0, _defineProperty2["default"])(_GLK, "glk_fileref_does_file_exist", glk_fileref_does_file_exist), (0, _defineProperty2["default"])(_GLK, "glk_fileref_create_from_fileref", glk_fileref_create_from_fileref), (0, _defineProperty2["default"])(_GLK, "glk_put_char", glk_put_char), (0, _defineProperty2["default"])(_GLK, "glk_put_char_stream", glk_put_char_stream), (0, _defineProperty2["default"])(_GLK, "glk_put_string", glk_put_string), (0, _defineProperty2["default"])(_GLK, "glk_put_string_stream", glk_put_string_stream), (0, _defineProperty2["default"])(_GLK, "glk_put_buffer", glk_put_buffer), (0, _defineProperty2["default"])(_GLK, "glk_put_buffer_stream", glk_put_buffer_stream), (0, _defineProperty2["default"])(_GLK, "glk_set_style", glk_set_style), (0, _defineProperty2["default"])(_GLK, "glk_set_style_stream", glk_set_style_stream), (0, _defineProperty2["default"])(_GLK, "glk_get_char_stream", glk_get_char_stream), (0, _defineProperty2["default"])(_GLK, "glk_get_line_stream", glk_get_line_stream), (0, _defineProperty2["default"])(_GLK, "glk_get_buffer_stream", glk_get_buffer_stream), (0, _defineProperty2["default"])(_GLK, "glk_char_to_lower", glk_char_to_lower), (0, _defineProperty2["default"])(_GLK, "glk_char_to_upper", glk_char_to_upper), (0, _defineProperty2["default"])(_GLK, "glk_stylehint_set", DO_NOTHING), (0, _defineProperty2["default"])(_GLK, "glk_stylehint_clear", DO_NOTHING), (0, _defineProperty2["default"])(_GLK, "glk_select", glk_select), (0, _defineProperty2["default"])(_GLK, "glk_request_line_event", glk_request_line_event), (0, _defineProperty2["default"])(_GLK, "glk_request_char_event", glk_request_char_event), (0, _defineProperty2["default"])(_GLK, "glk_cancel_char_event", glk_cancel_char_event), (0, _defineProperty2["default"])(_GLK, "glk_schannel_iterate", DO_NOTHING), (0, _defineProperty2["default"])(_GLK, "glk_buffer_to_lower_case_uni", glk_buffer_to_lower_case_uni), (0, _defineProperty2["default"])(_GLK, "glk_buffer_to_upper_case_uni", glk_buffer_to_upper_case_uni), (0, _defineProperty2["default"])(_GLK, "glk_buffer_to_title_case_uni", glk_buffer_to_title_case_uni), (0, _defineProperty2["default"])(_GLK, "glk_buffer_canon_decompose_uni", glk_buffer_canon_decompose_uni), (0, _defineProperty2["default"])(_GLK, "glk_buffer_canon_normalize_uni", glk_buffer_canon_normalize_uni), (0, _defineProperty2["default"])(_GLK, "glk_put_char_uni", glk_put_char_uni), (0, _defineProperty2["default"])(_GLK, "glk_put_string_uni", glk_put_string_uni), (0, _defineProperty2["default"])(_GLK, "glk_put_buffer_uni", glk_put_buffer_uni), (0, _defineProperty2["default"])(_GLK, "glk_put_char_stream_uni", glk_put_char_stream_uni), (0, _defineProperty2["default"])(_GLK, "glk_put_string_stream_uni", glk_put_string_stream_uni), (0, _defineProperty2["default"])(_GLK, "glk_put_buffer_stream_uni", glk_put_buffer_stream_uni), (0, _defineProperty2["default"])(_GLK, "glk_get_char_stream_uni", glk_get_char_stream_uni), (0, _defineProperty2["default"])(_GLK, "glk_get_buffer_stream_uni", glk_get_buffer_stream_uni), (0, _defineProperty2["default"])(_GLK, "glk_get_line_stream_uni", glk_get_line_stream_uni), (0, _defineProperty2["default"])(_GLK, "glk_stream_open_memory_uni", glk_stream_open_memory_uni), (0, _defineProperty2["default"])(_GLK, "glk_request_char_event_uni", glk_request_char_event_uni), (0, _defineProperty2["default"])(_GLK, "glk_request_line_event_uni", glk_request_line_event_uni), (0, _defineProperty2["default"])(_GLK, "glk_set_echo_line_event", glk_set_echo_line_event), (0, _defineProperty2["default"])(_GLK, "glk_set_terminators_line_event", glk_set_terminators_line_event), (0, _defineProperty2["default"])(_GLK, "glk_current_time", glk_current_time), (0, _defineProperty2["default"])(_GLK, "glk_current_simple_time", glk_current_simple_time), (0, _defineProperty2["default"])(_GLK, "glk_time_to_date_utc", glk_time_to_date_utc), (0, _defineProperty2["default"])(_GLK, "glk_time_to_date_local", glk_time_to_date_local), (0, _defineProperty2["default"])(_GLK, "glk_simple_time_to_date_utc", glk_simple_time_to_date_utc), (0, _defineProperty2["default"])(_GLK, "glk_simple_time_to_date_local", glk_simple_time_to_date_local), (0, _defineProperty2["default"])(_GLK, "glk_date_to_time_utc", glk_date_to_time_utc), (0, _defineProperty2["default"])(_GLK, "glk_date_to_time_local", glk_date_to_time_local), (0, _defineProperty2["default"])(_GLK, "glk_date_to_simple_time_utc", glk_date_to_simple_time_utc), (0, _defineProperty2["default"])(_GLK, "glk_date_to_simple_time_local", glk_date_to_simple_time_local), (0, _defineProperty2["default"])(_GLK, "sendChar", sendChar), (0, _defineProperty2["default"])(_GLK, "sendLine", sendLine), _GLK);
window.Glk = new Proxy(GLK, {
  get: function get(obj, prop) {
    return prop in obj ? obj[prop] : function () {
