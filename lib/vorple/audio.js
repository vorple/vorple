...
exports.currentMusicPlaying = currentMusicPlaying;
exports.fadeOut = fadeOut;
exports.isAudioPlaying = isAudioPlaying;
exports.isEffectPlaying = isEffectPlaying;
exports.isElementPlaying = isElementPlaying;
...
function fadeOut(element, duration, callback) {
  var tick = 50; // how often the volume is changed

  var $sound = $(element);
  var sound = $sound.get(0);

  var runCallbackIfExists = function runCallbackIfExists(remove) {
    if (remove) {
      $sound.remove();
    }
  ...
  var delta = tick / duration; // how much to change the volume on each tick
...
function isMusicPlaying() {
  var $music = $('.vorple-music'); // if the audio element doesn't exist, music is never playing
...
function playMusic(url) {
  ...
  var $music = $('.vorple-music');
  var looping = options.looping,
      restart = options.restart;
  clearTimeout(musicPauseTimer); // check if this specific track is already playing

  if (!restart && ($music.length > 0 && ($music.attr('src') === url || $music.data('resourceurl') === url) || $music.length === 0 && musicQueue.length > 0 && musicQueue[0].url === url)) {
    // if the music is fading out, stop the fadeout and continue
    clearTimeout($music.data("fadeOutTimer"));
    $music.prop('volume', 1).data('stopping', false).prop('loop', !!looping).get(0).play();
    ...
    $music.remove();
    $('<audio class="vorple-audio vorple-music">').attr('src', (0, _file.resourceUrl)(url)).data('resourceurl', url).prop('loop', !!looping).appendTo('body').on('ended', timeNextTrack).get(0).play();
  }
}
/**
...
function playSound(url) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  var looping = !!options.looping;
  var $audio = $('<audio class="vorple-audio vorple-sound-effect">').attr('src', (0, _file.resourceUrl)(url)).data('resourceurl', url).prop('loop', looping).appendTo('body');
  ...
  audioElement.play(); // if the sound is not looping, remove the element from the DOM when it finishes playing
...
function setPlaylist(list) {
  ...
  var currentTrackUrl = currentMusicPlaying(); // if the playlist is a list of URLs, turn them into objects that have
  // the "looping" property

  var pl = list.map(function (item) {
  ...
  var currentlyPlayingIndex = restart ? -1 : pl.findIndex(function (track) {
    return track.url === currentTrackUrl;
  });
  var startFromIndex = currentlyPlayingIndex < 1 ? 0 : currentlyPlayingIndex;
