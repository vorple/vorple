/* parser.js - Parser-specific features */
 ...
 * @since 2.4
 */
define( 'parser', [ 'jquery', 'notify', 'core', 'html' ], function( $, notify, core, html ) {
    "use strict";
    ...
    self.openTag = function( tag, classes ) {
        ...
        var $tag = $( html.tag( tag, '', { classes: classes, id: id } ) );
    ...
    self.registerFilter = function( filter, options ) {
        var opt = $.extend( { priority: 0, name: 'filter_' + core.generateId(), type: 'output' }, options );
    ...
    $( document ).on( 'init.vorple', function() {
        if( core.engine( 'parchment' ) ) {
            ...
            _container.parchment = core.getEngine().options.container;
