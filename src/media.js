 ...
 */
define( 'media', [ 'jquery', 'soundmanager2', 'core', 'html' ], function( $, SoundManager2, core, html ) {
    "use strict";
    ...
    var _playAudio = function( options ) {
        ...
        if( core.engine( 'undum' ) && !undum.isInteractive() ) {
            return;
        }
     ...
     * 
     * @example 
     * media.image( 'foo.jpg', { class: 'bar' } )
     *     == &lt;img src="media/image/foo.jpg" class="bar"&gt;
     *     
     * @param {String} file The filename of the image. 
     * If no path is specified, media.defaults.imagePath will be used.
    ...
    self.image = function( file, options ) {
        ...
        var opt = $.extend( {}, { src: html.url( file, imagePath ) }, options );
     ...
     *   media.mute({ music: true, sound: false });
     * 
     ...
     * Play a music file.
     * 
     * This is otherwise functionally identical to media.playSound(),
    ...
    self.playMusic = function( file, options ) {
        ...
        opt.url = html.url( file, opt.musicPath );                
    ...
    self.playSound = function( file, options ) {
        ...
        opt.url = html.url( file, opt.audioPath );                
    ...
    self.preloadImage = function( files, options ) {
        ...
        if( typeof files == 'string' ) {
            var image = new Image();
            image.src = html.url( files, opt.imagePath );
            return image;
        }
            ...
            $.each( files, function() {
                var image = new Image();
                image.src = html.url( this, opt.imagePath );
                images.push( image );
            } );
            return images;
        }
    ...
    $( document ).on( 'init.vorple', function() {
        ...
        var previousStatus = localStorage.getItem( 'vorpleMute' );
